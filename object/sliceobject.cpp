...
void slice_del(object* self){
    ...
    FPLDECREF(CAST_SLICE(self)->step);
}
...
object* slice_repr(object* self){
    ...
    s+=", ";
    s+=object_cstr(CAST_SLICE(self)->step);
    s+=")";
...
object* slice_cmp(object* self, object* other, uint8_t type){
    ...
    if (type==CMP_EQ){
        ...
        && istrue(object_cmp(CAST_SLICE(self)->step, CAST_SLICE(other)->step, CMP_EQ))){
