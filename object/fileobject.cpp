...
object* file_new(object* type, object* args, object* kwargs){
    ...
    if (!object_istype(name->type, &StrType)){
        vm_add_err(&TypeError, vm, "Expected string object for name, got type '%s'", name->type->name->c_str());
        return NULL;
    }
    object* mode_=list_index_int(args, 1);
    if (!object_istype(mode_->type, &StrType)){
        vm_add_err(&TypeError, vm, "Expected string object for mode, got type '%s'", mode_->type->name->c_str());
        return NULL;
    }
    ...
    if (len!=3){
        ...
        if (!object_istype(encoding_->type, &StrType)){
            vm_add_err(&TypeError, vm, "Expected string object for encoding, got type '%s'", encoding_->type->name->c_str());
            return NULL;
        }
    }
...
object* file_new_fromfile(object* name, char* mode){
    if (!object_istype(name->type, &StrType)){
        vm_add_err(&TypeError, vm, "Expected string type name, got type '%s'", name->type->name->c_str());
        return NULL;
    }
    string s=*CAST_STRING(name)->val;
