...
object* list_store_slice(object* self, object* idx, object* val){
    object* start=CAST_SLICE(idx)->start;
    object* end=CAST_SLICE(idx)->end;
    object* step=CAST_SLICE(idx)->step;
...
object* list_del_slice(object* self, object* index){
    ...
    CAST_LIST(self)->size=CAST_LIST(self)->size-index_v;
...
object* list_set(object* self, object* idx, object* val){
    if (object_istype(idx->type, &SliceType)){
        if (val==NULL){
            if (list_del_slice(self, idx)==NULL){
                return NULL;
            }
            return new_none();
        }
        if (list_store_slice(self, idx,val)==NULL){
            return NULL;
        }
        return new_none();
    } 
...
void list_append(object* self, object* obj){
    ...
    FPLINCREF(obj);
    CAST_LIST(self)->array[CAST_LIST(self)->size++]=obj;
...
object* list_mul(object* self, object* other){
    ...
    for (int i=1; i<nrepeat; i++){
        for (int a=0; a<orig_size; a++){
            object* o=CAST_LIST(self)->array[a];
            FPLINCREF(o);
            CAST_LIST(self)->array[a+(i*orig_size)]=o;
        }
    }
    return self;
}
...
object* list_replace_meth(object* selftp, object* args, object* kwargs){
    ...
    for (size_t i=0; i<CAST_LIST(self)->size; i++){
        if (istrue(object_cmp(CAST_LIST(self)->array[i], replace, CMP_EQ))){
            ...
            FPLINCREF(replacewi);
            CAST_LIST(self)->array[i]=replacewi;
            return self;
        }
    }
