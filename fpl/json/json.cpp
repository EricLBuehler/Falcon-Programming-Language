...
object* json_decode_object(const nx_json* json){
    ...
    while (item){
        switch (item->type){
            case NX_JSON_NULL:
                dict_set(dict, str_new_fromstr(string(item->key)), new_none());
                break;
            case NX_JSON_STRING:
                dict_set(dict, str_new_fromstr(string(item->key)), str_new_fromstr(string(item->text_value)));
                break;
            case NX_JSON_INTEGER:
                dict_set(dict, str_new_fromstr(string(item->key)), new_int_fromint(item->num.s_value));
                break;
            case NX_JSON_DOUBLE:
                dict_set(dict, str_new_fromstr(string(item->key)), new_float_fromdouble(item->num.dbl_value));
                break;
            case NX_JSON_BOOL:
                dict_set(dict, str_new_fromstr(string(item->key)), item->num.u_value? new_bool_true() : new_bool_false());
                break;
            case NX_JSON_OBJECT:
                dict_set(dict, str_new_fromstr(string(item->key)), json_decode_object(item));
                break;
            case NX_JSON_ARRAY:
                dict_set(dict, str_new_fromstr(string(item->key)), json_decode_array(item));
                break;
        }
        item=item->next;
    }
...
object* new_json_module(){
    ...
    dict_set(dict, str_new_fromstr("decode"), decode);
    FPLDECREF(decode);
    ...
    dict_set(dict, str_new_fromstr("encode"), encode);
    FPLDECREF(encode);
